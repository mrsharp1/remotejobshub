import React, { useState } from 'react'
import { TrendingUp, ArrowUpRight, ArrowDownRight } from 'lucide-react'

interface RevenueOverviewProps {
  payments: any[]
}

type Timeframe = 'daily' | 'weekly' | 'monthly' | 'yearly'

export const RevenueOverview: React.FC<RevenueOverviewProps> = ({ payments: _payments }) => {
  const [timeframe, setTimeframe] = useState<Timeframe>('monthly')
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null)

  // High fidelity chart mock datasets for rendering smooth curves
  const datasets: Record<Timeframe, { label: string; value: number }[]> = {
    daily: [
      { label: '00:00', value: 1200 },
      { label: '04:00', value: 800 },
      { label: '08:00', value: 2400 },
      { label: '12:00', value: 3900 },
      { label: '16:00', value: 3100 },
      { label: '20:00', value: 4500 },
      { label: '24:00', value: 5200 },
    ],
    weekly: [
      { label: 'Mon', value: 8500 },
      { label: 'Tue', value: 12400 },
      { label: 'Wed', value: 11100 },
      { label: 'Thu', value: 14500 },
      { label: 'Fri', value: 16800 },
      { label: 'Sat', value: 13200 },
      { label: 'Sun', value: 18900 },
    ],
    monthly: [
      { label: 'Jan', value: 38000 },
      { label: 'Feb', value: 42000 },
      { label: 'Mar', value: 49000 },
      { label: 'Apr', value: 47000 },
      { label: 'May', value: 58000 },
      { label: 'Jun', value: 68000 },
      { label: 'Jul', value: 74000 },
      { label: 'Aug', value: 71000 },
      { label: 'Sep', value: 85000 },
      { label: 'Oct', value: 92000 },
      { label: 'Nov', value: 98000 },
      { label: 'Dec', value: 115000 },
    ],
    yearly: [
      { label: '2022', value: 340000 },
      { label: '2023', value: 580000 },
      { label: '2024', value: 890000 },
      { label: '2025', value: 1240000 },
      { label: '2026', value: 1680000 },
    ],
  }

  const activeData = datasets[timeframe]
  const values = activeData.map((d) => d.value)
  const maxValue = Math.max(...values, 1000)
  const minValue = Math.min(...values, 0)
  const range = maxValue - minValue

  // SVG dimensions
  const width = 600
  const height = 180
  const padding = 20

  // Coordinate mapper
  const getCoords = (index: number, value: number) => {
    const x = padding + (index / (activeData.length - 1)) * (width - padding * 2)
    const y = height - padding - ((value - minValue) / range) * (height - padding * 2)
    return { x, y }
  }

  // Generate SVG path using coordinates
  const points = activeData.map((d, i) => getCoords(i, d.value))
  
  // Bezier curve path generator
  let pathD = ''
  if (points.length > 0) {
    pathD = `M ${points[0].x} ${points[0].y}`
    for (let i = 0; i < points.length - 1; i++) {
      const p0 = points[i]
      const p1 = points[i + 1]
      const cpX1 = p0.x + (p1.x - p0.x) / 2
      const cpY1 = p0.y
      const cpX2 = p0.x + (p1.x - p0.x) / 2
      const cpY2 = p1.y
      pathD += ` C ${cpX1} ${cpY1}, ${cpX2} ${cpY2}, ${p1.x} ${p1.y}`
    }
  }

  // Closed path for gradient area fill
  const areaD = pathD
    ? `${pathD} L ${points[points.length - 1].x} ${height - padding} L ${points[0].x} ${height - padding} Z`
    : ''

  return (
    <div className="rounded-2xl border border-white/5 bg-slate-900/60 p-6 shadow-xl backdrop-blur-xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <TrendingUp className="h-5 w-5 text-indigo-400" />
            <h2 className="font-heading text-lg font-bold text-white">Revenue Analytics</h2>
          </div>
          <p className="text-xs text-slate-400">Platform commissions & transaction overview</p>
        </div>

        {/* Timeframe Switcher */}
        <div className="flex items-center gap-1.5 rounded-xl border border-white/5 bg-slate-950 p-1">
          {(['daily', 'weekly', 'monthly', 'yearly'] as Timeframe[]).map((t) => (
            <button
              key={t}
              onClick={() => {
                setTimeframe(t)
                setHoveredIndex(null)
              }}
              className={`rounded-lg px-3 py-1.5 text-[10px] font-bold uppercase transition-all ${
                timeframe === t
                  ? 'bg-indigo-600 text-white shadow-lg'
                  : 'text-slate-400 hover:bg-white/5 hover:text-white'
              }`}
            >
              {t}
            </button>
          ))}
        </div>
      </div>

      {/* Main Revenue Stats */}
      <div className="mt-6 grid grid-cols-2 gap-4 border-b border-white/5 pb-6 sm:grid-cols-4">
        <div>
          <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
            Average Order Value
          </span>
          <h3 className="mt-1 font-mono text-xl font-bold text-white">₦248,500.00</h3>
          <span className="flex items-center gap-0.5 text-[10px] font-semibold text-emerald-400">
            <ArrowUpRight className="h-3 w-3" /> +4.2% AOV
          </span>
        </div>
        <div>
          <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
            Total Commission Yield
          </span>
          <h3 className="mt-1 font-mono text-xl font-bold text-white">10.0%</h3>
          <span className="text-[10px] text-slate-400">Standard rate applied</span>
        </div>
        <div>
          <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
            Success Payout rate
          </span>
          <h3 className="mt-1 font-mono text-xl font-bold text-white">99.85%</h3>
          <span className="flex items-center gap-0.5 text-[10px] font-semibold text-emerald-400">
            <ArrowUpRight className="h-3 w-3" /> Industry grade
          </span>
        </div>
        <div>
          <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
            Disputed Escrow Rate
          </span>
          <h3 className="mt-1 font-mono text-xl font-bold text-white">0.12%</h3>
          <span className="flex items-center gap-0.5 text-[10px] font-semibold text-rose-400">
            <ArrowDownRight className="h-3 w-3" /> -0.05% MoM
          </span>
        </div>
      </div>

      {/* SVG Chart display */}
      <div className="relative mt-6">
        <svg
          viewBox={`0 0 ${width} ${height}`}
          className="w-full overflow-visible"
        >
          <defs>
            <linearGradient id="chartGlow" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#4f46e5" stopOpacity="0.4" />
              <stop offset="100%" stopColor="#4f46e5" stopOpacity="0.0" />
            </linearGradient>
          </defs>

          {/* Grid Lines */}
          <line x1={padding} y1={padding} x2={width - padding} y2={padding} stroke="rgba(255,255,255,0.03)" strokeWidth={1} />
          <line x1={padding} y1={(height - padding * 2) / 2 + padding} x2={width - padding} y2={(height - padding * 2) / 2 + padding} stroke="rgba(255,255,255,0.03)" strokeWidth={1} />
          <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} stroke="rgba(255,255,255,0.05)" strokeWidth={1} />

          {/* Area Fill */}
          {areaD && (
            <path d={areaD} fill="url(#chartGlow)" />
          )}

          {/* Path Line */}
          {pathD && (
            <path
              d={pathD}
              fill="none"
              stroke="#6366f1"
              strokeWidth={3}
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          )}

          {/* Data Nodes */}
          {points.map((p, idx) => (
            <g key={idx}>
              <circle
                cx={p.x}
                cy={p.y}
                r={hoveredIndex === idx ? 6 : 4}
                className={`cursor-pointer transition-all duration-150 ${
                  hoveredIndex === idx ? 'fill-indigo-400 stroke-white stroke-2' : 'fill-slate-900 stroke-indigo-500 stroke-2'
                }`}
                onMouseEnter={() => setHoveredIndex(idx)}
                onMouseLeave={() => setHoveredIndex(null)}
              />
            </g>
          ))}
        </svg>

        {/* Dynamic Premium Tooltip */}
        {hoveredIndex !== null && (
          <div
            className="pointer-events-none absolute z-20 rounded-xl border border-white/10 bg-slate-950/90 px-3 py-2 shadow-2xl backdrop-blur-md transition-all duration-150"
            style={{
              left: `${(points[hoveredIndex].x / width) * 100}%`,
              top: `${(points[hoveredIndex].y / height) * 100 - 35}%`,
              transform: 'translateX(-50%)',
            }}
          >
            <span className="block text-[8px] font-bold uppercase tracking-wider text-slate-400">
              {activeData[hoveredIndex].label}
            </span>
            <span className="mt-0.5 block font-mono text-xs font-bold text-white">
              ${activeData[hoveredIndex].value.toLocaleString()}
            </span>
          </div>
        )}
      </div>

      {/* X Axis Labels */}
      <div className="mt-3 flex justify-between px-3 text-[9px] font-bold text-slate-500 uppercase">
        {activeData.map((d, idx) => (
          <span key={idx}>{d.label}</span>
        ))}
      </div>
    </div>
  )
}
