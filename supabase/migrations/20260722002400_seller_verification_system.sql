-- Migration to establish KYC and Seller Verifications tables
...
CREATE TABLE IF NOT EXISTS public.seller_verifications (
  ...
  status TEXT NOT NULL CHECK (status IN ('pending', 'under_review', 'approved', 'rejected', 'requires_more_info')) DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS public.verification_audit_logs (
  ...
  action TEXT NOT NULL CHECK (action IN ('submit', 'review', 'approve', 'reject', 'resubmit', 'request_documents')),
  notes TEXT,
  created_at TIMESTAMPTZ DEFAULT now()
);
...
DROP POLICY IF EXISTS "Select docs policy" ON public.verification_documents;
CREATE POLICY "Select docs policy" ON public.verification_documents
  FOR SELECT USING (
    EXISTS (
...
DROP POLICY IF EXISTS "Insert docs policy" ON public.verification_documents;
CREATE POLICY "Insert docs policy" ON public.verification_documents
...
DROP POLICY IF EXISTS "Delete docs policy" ON public.verification_documents;
CREATE POLICY "Delete docs policy" ON public.verification_documents
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.seller_verifications 
      WHERE id = verification_documents.verification_id 
...
-- RLS Policies for verification_audit_logs
DROP POLICY IF EXISTS "Select audit policy" ON public.verification_audit_logs;
CREATE POLICY "Select audit policy" ON public.verification_audit_logs
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.seller_verifications 
      WHERE id = verification_audit_logs.verification_id 
      AND (user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin'))
    )
  );

DROP POLICY IF EXISTS "Insert audit policy" ON public.verification_audit_logs;
CREATE POLICY "Insert audit policy" ON public.verification_audit_logs
  FOR INSERT WITH CHECK (
